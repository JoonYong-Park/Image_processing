...
int main_1() {
	...
	for (int i = 0; i < 256; i++)
		for (int j = 0; j < 256; j++)
			//Writebuf[j][i] = Readbuf[256 - j][256 - i]; // Readbuf의 값을 뒤집어 Writebuf에 저장(수직 뒤집기)
			Writebuf[j][i] = Readbuf[256 - j][256 - i]; // Readbuf의 값을 뒤집어 Writebuf에 저장(상하좌우 뒤집기)


	// 영상 저장
	//fp = fopen("LENA_256x256_8bit_left_right.raw", "wb"); // "LENA_256x256_8bit_new.raw" 파일을 쓰기 모드로 열기
